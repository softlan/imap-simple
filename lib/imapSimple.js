    ...
    return new Promise(function (resolve, reject) {
        ...
        self.imap.search(searchCriteria, function (err, uids) {
            ...
            function fetchOnMessage(message, seqNo) {
                getMessage.getMessage(message).then(function (message) { // Октаэдр.Лебедев
                    message.seqNo = seqNo;
 ...
 * @param {object} set Дополнительные настройки парсера
 ...
 * @memberof ImapSimple
 */
ImapSimple.prototype.getPartData = function (message, part, set, callback) { // Октаэдр.Лебедев
    var self = this;
    ...
    return new Promise(function (resolve, reject) {
        ...
        function fetchOnMessage(msg) {
            // Октаэдр.Лебедев +
            if (set && set.advConvert && set.advConvert === 1) {
                getMessage.getMessage2(msg).then(function (result) { // Октаэдр.Лебедев
                    ...
                    if (encoding === 'BASE64') {
                        resolve(new Buffer(data.toString('utf8'), 'base64')); // Октаэдр.Лебедев
                        return;
                    }
                    ...
                    if (encoding === 'QUOTED-PRINTABLE') {
                        if (part.params && part.params.charset &&
                            ...
                            resolve((new Buffer(utf8.decode(qp.decode(data.toString('utf8'))))).toString()); // Октаэдр.Лебедев
                        } else {
                            resolve(iconvlite.decode((new Buffer(qp.decode(data.toString('utf8')))).toString(), part.params.charset)); // Октаэдр.Лебедев
                        }
                        return;
                    }

                    if (encoding === '7BIT') {
                        resolve((new Buffer(data.toString('utf8'))).toString('ascii')); // Октаэдр.Лебедев
                        return;
                    }
            ...
            // Октаэдр.Лебедев -
                ...
                getMessage.getMessage(msg).then(function (result) { // Октаэдр.Лебедев
                    if (result.parts.length !== 1) {
                        reject(new Error('Got ' + result.parts.length + ' parts, should get 1'));
                        return;
                    }
                    ...
                    if (encoding === 'QUOTED-PRINTABLE') {
                        if (part.params && part.params.charset &&
                            ...
                            resolve(iconvlite.decode((new Buffer(qp.decode(data))).toString(), part.params.charset));
                        }
                        return;
                    }
